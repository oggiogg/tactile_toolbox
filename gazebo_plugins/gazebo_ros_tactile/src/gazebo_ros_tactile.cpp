...
# endif
  ...
  //taxels_.push_back(Taxel(taxel->position, taxel->normal));
  ...
  //SensorMap
	  ...
	  std::map<std::string, std::map<index_t, index_t> > mappings_
	  void TaxelGroup::addTaxel(const Taxel &taxel)
{
	taxels_.push_back(taxel);
}

static boost::shared_ptr<TaxelGroup> //Vorher: typedef TaxelGroupPtr&
getGroup(std::map<std::string, TaxelGroupPtr> &groups, const std::string &frame) {
	std::map<std::string, boost::shared_ptr<TaxelGroup>>::iterator it = groups.find(frame);
	if (it != groups.end()) return it->second;
	auto res = groups.insert(std::make_pair(frame, boost::shared_ptr<TaxelGroup>(new TaxelGroup(frame))));
	return res.first->second;
}

void addTaxels(const urdf::SensorConstSharedPtr &sensor) {
	std::map<std::string, TaxelGroupPtr> mapping;
...
//
	  ...
	  for (auto it = sensors.begin(), end = sensors.end(); it != end; ++it) {
		...
		boost::shared_ptr<TaxelGroup> &group = getGroup(result, it->second->parent_link_);
		//group->addTaxels(it->second);
	  }*/
	  //size_t test=sensors.size();
	  //ROS_INFO_STREAM_ONCE("sensor map size" << test);
	  //~ for (auto it = sensors.begin(), end = sensors.end(); it != end; ++it) {
		//~ urdf::tactile::TactileSensorConstSharedPtr sensor = urdf::tactile::tactile_sensor_cast(it->second);
		//~ if (!sensor) continue;  // some other sensor than tactile
