...
void GazeboRosTactile::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf) {
  ...
  if (this->local_link_ == NULL && this->frame_name_ != "world" &&
    ...
    // lock(*gazebo::Simulator::Instance()->GetMRMutex());
    ...
    for (physics::Model_V::iterator iter = all_models.begin();
      ...
      if (*iter)
        this->local_link_ = boost::dynamic_pointer_cast<physics::Link>(
            (*iter)->GetLink(this->frame_name_));
      if (this->local_link_)
        break;
    }
  ...
  ROS_INFO_STREAM_ONCE(" normalX" << taxelNormals[0][0].x << " normalY"
                                  << taxelNormals[0][0].y << " normalZ"
                                  << taxelNormals[0][0].z);
  ROS_INFO_STREAM_ONCE(" normalX" << taxelNormals[0][1].x << " normalY"
                                  << taxelNormals[0][1].y << " normalZ"
                                  << taxelNormals[0][1].z);
  ROS_INFO_STREAM_ONCE(" normalX" << taxelNormals[0][2].x << " normalY"
                                  << taxelNormals[0][2].y << " normalZ"
                                  << taxelNormals[0][2].z);
  ROS_INFO_STREAM_ONCE(" normalX" << taxelNormals[0][3].x << " normalY"
                                  << taxelNormals[0][3].y << " normalZ"
                                  << taxelNormals[0][3].z);
  ROS_INFO_STREAM_ONCE(" normalX" << taxelNormals[0][4].x << " normalY"
                                  << taxelNormals[0][4].y << " normalZ"
                                  << taxelNormals[0][4].z);
  ROS_INFO_STREAM_ONCE(" normalX" << taxelNormals[0][5].x << " normalY"
                                  << taxelNormals[0][5].y << " normalZ"
                                  << taxelNormals[0][5].z);
  ROS_INFO_STREAM_ONCE(" normalX" << taxelNormals[0][6].x << " normalY"
                                  << taxelNormals[0][6].y << " normalZ"
                                  << taxelNormals[0][6].z);
  ROS_INFO_STREAM_ONCE(" normalX" << taxelNormals[0][7].x << " normalY"
                                  << taxelNormals[0][7].y << " normalZ"
                                  << taxelNormals[0][7].z);
  ROS_INFO_STREAM_ONCE(" normalX" << taxelNormals[0][8].x << " normalY"
                                  << taxelNormals[0][8].y << " normalZ"
                                  << taxelNormals[0][8].z);
  ROS_INFO_STREAM_ONCE(" normalX" << taxelNormals[0][9].x << " normalY"
                                  << taxelNormals[0][9].y << " normalZ"
                                  << taxelNormals[0][9].z);
  ROS_INFO_STREAM_ONCE(" normalX" << taxelNormals[0][10].x << " normalY"
                                  << taxelNormals[0][10].y << " normalZ"
                                  << taxelNormals[0][10].z);
  ROS_INFO_STREAM_ONCE(" normalX" << taxelNormals[0][11].x << " normalY"
                                  << taxelNormals[0][11].y << " normalZ"
                                  << taxelNormals[0][11].z);

  ROS_INFO_STREAM_ONCE("Taxel " << 0 << " " << numOfTaxels[0] << "\n");
  ROS_INFO_STREAM_ONCE(" originX" << taxelPositions[0][0].x << " originY"
                                  << taxelPositions[0][0].y << " originZ"
                                  << taxelPositions[0][0].z);
  ROS_INFO_STREAM_ONCE(" originX" << taxelPositions[0][1].x << " originY"
                                  << taxelPositions[0][1].y << " originZ"
                                  << taxelPositions[0][1].z);
  ROS_INFO_STREAM_ONCE(" originX" << taxelPositions[0][2].x << " originY"
                                  << taxelPositions[0][2].y << " originZ"
                                  << taxelPositions[0][2].z);
  ROS_INFO_STREAM_ONCE(" originX" << taxelPositions[0][3].x << " originY"
                                  << taxelPositions[0][3].y << " originZ"
                                  << taxelPositions[0][3].z);
  ROS_INFO_STREAM_ONCE(" originX" << taxelPositions[0][4].x << " originY"
                                  << taxelPositions[0][4].y << " originZ"
                                  << taxelPositions[0][4].z);
  ROS_INFO_STREAM_ONCE(" originX" << taxelPositions[0][5].x << " originY"
                                  << taxelPositions[0][5].y << " originZ"
                                  << taxelPositions[0][5].z);
  ROS_INFO_STREAM_ONCE(" originX" << taxelPositions[0][6].x << " originY"
                                  << taxelPositions[0][6].y << " originZ"
                                  << taxelPositions[0][6].z);
  ROS_INFO_STREAM_ONCE(" originX" << taxelPositions[0][7].x << " originY"
                                  << taxelPositions[0][7].y << " originZ"
                                  << taxelPositions[0][7].z);
  ROS_INFO_STREAM_ONCE(" originX" << taxelPositions[0][8].x << " originY"
                                  << taxelPositions[0][8].y << " originZ"
                                  << taxelPositions[0][8].z);
  ROS_INFO_STREAM_ONCE(" originX" << taxelPositions[0][9].x << " originY"
                                  << taxelPositions[0][9].y << " originZ"
                                  << taxelPositions[0][9].z);
  ROS_INFO_STREAM_ONCE(" originX" << taxelPositions[0][10].x << " originY"
                                  << taxelPositions[0][10].y << " originZ"
                                  << taxelPositions[0][10].z);
  ROS_INFO_STREAM_ONCE(" originX" << taxelPositions[0][11].x << " originY"
                                  << taxelPositions[0][11].y << " originZ"
                                  << taxelPositions[0][11].z);
...
void GazeboRosTactile::OnContact() {
  ...
  if (local_link_) {
    frame_pose =
        local_link_
            ->GetWorldPose(); //-this->myBody->GetCoMPose();->GetDirtyPose();
  ...
  for (unsigned int i = 0; i < contactsPacketSize; ++i) {
    ...
    for (unsigned int j = 0; j < contactGroupSize; ++j) {
      ...
      math::Vector3 force = frame_rot.RotateVectorReverse(
          math::Vector3(contact.wrench(j).body_1_wrench().force().x(),
      ...
      math::Vector3 torque = frame_rot.RotateVectorReverse(
          math::Vector3(contact.wrench(j).body_1_wrench().torque().x(),
